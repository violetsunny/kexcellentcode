...
<li>关系： ① -> ② <- ③   ④ -> ⑤ </li></br>
这是一个指向父节点的树。</br>
如果增加1和3朋友关系，可以找1和3的根是否相同，相同不需要相连。不然就会成了环。</br>
如果添加3和4朋友关系，可以找3和4的根是否相同，不同则将他们的根2和5相连。</br>
可以用数组来表示这个树的关系，初始时刻数组中都是-1，将一个数的根设为另一个数的根的下标就行。</br>
比如上面的关系就是：</br>
-1 -1 -1 -1 -1  </br>
1 &nbsp;-1 &nbsp;1 &nbsp;4 &nbsp;-1   &nbsp;&nbsp;1改为2的下标，2不动，3改为2的下标，4改为5的下标，5不动。</br>
然后我们只需要查找到-1的个数即可。</br></br>

<li>假设朋友关系是：1和2是朋友，2和3是朋友，3和4是朋友，4和5是朋友。就会变成：① -> ② -> ③ -> ④ -> ⑤ </li></br>
会退化成一个链表，所以在合并的时候，需要将矮树合并到高树上去，也就是按秩(高度)合并。</br>
